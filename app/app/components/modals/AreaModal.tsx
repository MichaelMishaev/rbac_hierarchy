...
export default function AreaModal({
  ...
  const validate = (): boolean => {
    ...
    if (!formData.regionName.trim()) {
      newErrors.regionName = 'שם מחוז הוא שדה חובה';
    } else if (formData.regionName.trim().length < 2) {
      newErrors.regionName = 'שם מחוז חייב להכיל לפחות 2 תווים';
    }

    // Code is auto-generated, just ensure it exists
    if (!formData.regionCode.trim()) {
      newErrors.regionCode = 'שגיאה ביצירת קוד מחוז';
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || 'שגיאה בשמירת המחוז');
      }
    } catch (err) {
      console.error('Error submitting area:', err);
      setError('שגיאה בשמירת המחוז. אנא נסה שוב.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <Typography variant="h5" sx={{ fontWeight: 700, color: 'text.primary' }}>
              {mode === 'create' ? t('newArea') : 'עריכת מחוז'}
            </Typography>
            <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>
              {mode === 'create' ? 'הוסף מחוז חדש למערכת' : 'ערוך את פרטי המחוז'}
            </Typography>
          </Box>
        </Box>
      </DialogTitle>
          ...
          >
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <TextField
                ...
                required
                autoFocus
                placeholder="הכנס שם מחוז בעברית"
                sx={{
              ...
              {mode === 'edit' && (
                <TextField
                  ...
                  fullWidth
                  disabled
                  helperText="לא ניתן לשנות קוד מחוז קיים"
                  inputProps={{
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField
                    ...
                    error={!!errors.userId}
                    helperText={errors.userId || 'מחוז יכול להיות ללא מנהל. רק משתמשים עם תפקיד "מנהל מחוז" זמינים'}
                    sx={{
                      '& .MuiOutlinedInput-root': {
                        borderRadius: '12px',
                        backgroundColor: 'white',
              ...
              {availableUsers.length === 0 && (
                <Alert severity="info" sx={{ mt: 1 }}>
                  אין משתמשים זמינים עם תפקיד &quot;מנהל מחוז&quot;. המחוז יווצר ללא מנהל ותוכל להקצות מנהל מאוחר יותר.
                </Alert>
              )}
            </Box>
          </Box>
          ...
          >
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <TextField
                ...
                multiline
                rows={3}
                placeholder="תיאור המחוז, אחריות, סמכויות..."
                sx={{
                  '& .MuiOutlinedInput-root': {
                    borderRadius: '12px',
                    backgroundColor: 'white',
              ...
              <FormControlLabel
                ...
                label={
                  <Box>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      מחוז פעיל זמין לשיוך ערים ופעילות
                    </Typography>
                  </Box>
                }
              />
