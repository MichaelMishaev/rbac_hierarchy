#!/usr/bin/env node
/**
 * One-time script to create voters tables in production
 * Run with: node scripts/run-voters-migration.js
 */
...
const fkeys = [
  `ALTER TABLE "voters" ADD CONSTRAINT IF NOT EXISTS "voters_assigned_city_id_fkey" FOREIGN KEY ("assigned_city_id") REFERENCES "cities"("id") ON DELETE SET NULL ON UPDATE CASCADE`,
  `ALTER TABLE "voters" ADD CONSTRAINT IF NOT EXISTS "voters_inserted_by_user_id_fkey" FOREIGN KEY ("inserted_by_user_id") REFERENCES "users"("id") ON DELETE RESTRICT ON UPDATE CASCADE`,
  `ALTER TABLE "voter_edit_history" ADD CONSTRAINT IF NOT EXISTS "voter_edit_history_voter_id_fkey" FOREIGN KEY ("voter_id") REFERENCES "voters"("id") ON DELETE CASCADE ON UPDATE CASCADE`,
];

async function main() {
  console.log('🚀 Starting voters migration...');
